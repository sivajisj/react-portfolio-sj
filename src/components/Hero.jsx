...
const Hero = () => {
  ...
  return (
    <div name="home" className="w-full min-h-screen flex items-center pt-[76px]">
      <div className="max-w-[1200px] w-full mx-auto px-6 grid lg:grid-cols-[1.1fr_0.9fr] gap-14 items-center">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <div className="h-[44px] sm:h-[52px] mt-3 overflow-hidden">
            <motion.h2
              key={roleIndex}
          ...
          <p className="text-slate-400 py-6 max-w-[580px] leading-relaxed">
            I build across three pillars:{' '}
          ...
          <div className="flex flex-wrap gap-4">
            <Link to="work" smooth offset={-76} duration={500}>
              <button className="group bg-gradient-to-r from-accent-cyan to-accent text-white font-semibold px-6 py-3 rounded-lg flex items-center gap-2 hover:shadow-lg hover:shadow-accent-cyan/20 transition-all duration-300">
                View Projects
        ...
        >
          <div className="glass rounded-2xl p-6 font-mono text-sm leading-relaxed shadow-2xl shadow-black/50 animate-float">
            <div className="flex gap-2 mb-5">
              <span className="w-3 h-3 rounded-full bg-red-400/70" />
              <span className="w-3 h-3 rounded-full bg-yellow-400/70" />
              <span className="w-3 h-3 rounded-full bg-emerald-400/70" />
              <span className="ml-2 text-slate-600 text-xs">sivaji.rs</span>
            </div>
            <pre className="text-slate-300 whitespace-pre-wrap">
{`#[program]
pub mod sivaji_dev \\{
    pub fn build(ctx: Context<Init>)
        -> Result<()>
    \\{
        let dev = &mut ctx.accounts.me;

        dev.solana = vec![
            "Anchor", "SPL", "PDAs", "CPIs",
        ];
        dev.evm = vec!["Solidity", "Hardhat"];
        dev.backend = vec!["Axum", "Tokio"];
        dev.focus  = "DePIN on Solana";
